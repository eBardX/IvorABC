...
extension ABCParseFunctions {
    ...
    func test_parseField_success() throws {
        expectFieldIsAlignedLyrics(try parseField("w:la la la"), "la la la")
        expectFieldIsArea(try parseField("A:London"), "London")
        expectFieldIsBook(try parseField("B:My Fakebook"), "My Fakebook")
        expectFieldIsComposer(try parseField("C:J.S. Bach"), "J.S. Bach")
        expectFieldIsContinuation(try parseField("+:more text"), "more text")
        expectFieldIsDiscography(try parseField("D:Collected Works"), "Collected Works")
        expectFieldIsFileURL(try parseField("F:https://example.com"), "https://example.com")
        expectFieldIsGroup(try parseField("G:Reels"), "Reels")
        expectFieldIsHistory(try parseField("H:Traditional"), "Traditional")
        expectFieldIsInstruction(try parseField("I:linebreak $"), "linebreak $")
        expectFieldIsKey(try parseField("K:G"))
        expectFieldIsLyrics(try parseField("W:do re mi"), "do re mi")
        expectFieldIsMacro(try parseField("m:~G3 = G{A}G2"), "~G3 = G{A}G2")
        expectFieldIsMeter(try parseField("M:4/4"))
        expectFieldIsNotes(try parseField("N:See also"), "See also")
        expectFieldIsOrigin(try parseField("O:Ireland"), "Ireland")
        expectFieldIsParts(try parseField("P:AABB"), "AABB")
        expectFieldIsRefNumber(try parseField("X:1"))
        expectFieldIsRemark(try parseField("r:editorial note"), "editorial note")
        expectFieldIsRhythm(try parseField("R:Reel"), "Reel")
        expectFieldIsSource(try parseField("S:collected by ..."), "collected by ...")
        expectFieldIsSymbolLine(try parseField("s:!p! * * *"), "!p! * * *")
        expectFieldIsTempo(try parseField("Q:1/4=120"))
        expectFieldIsTitle(try parseField("T:My Tune"), "My Tune")
        expectFieldIsTranscription(try parseField("Z:John Doe"), "John Doe")
        expectFieldIsUnitNoteLength(try parseField("L:1/8"))
        expectFieldIsUserDefined(try parseField("U:~=!roll!"), "~=!roll!")
        expectFieldIsVoice(try parseField("V:1"))
    }
