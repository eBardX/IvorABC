...
extension ABCFormatterTests {
    ...
    func alignedLyrics_continuation_emitsHyphen() throws {
        ...
        #expect(try format(tunebook).contains("w:hel-lo\n"))
    }
    ...
    func alignedLyrics_hold_emitsUnderscore() throws {
        ...
        #expect(try format(tunebook).contains("w:long _\n"))
    }
    ...
    func alignedLyrics_syllableWithHyphen_escapesHyphen() throws {
        ...
        #expect(try format(tunebook).contains("w:foo\\-bar\n"))
    }
    ...
    func alignedLyrics_syllableWithInternalSpace_emitsTilde() throws {
        ...
        #expect(try format(tunebook).contains("w:how~sweet\n"))
    }
    ...
    func alignedLyrics_syllableWithPercent_escapesPercentSign() throws {
        ...
        #expect(try format(tunebook).contains("w:100\\% done\n"))
    }

    @Test
    func barLine_emitsVerbatim() throws {
        let note = makeNote(makePitch(.c, .natural, 4), makeLength(1, 8))
    ...
    func brokenRhythm_doubleRight_emitsVerbatim() throws {
        let c = makeNote(makePitch(.c, .natural, 4), makeLength(1, 8))
        let d = makeNote(makePitch(.d, .natural, 4), makeLength(1, 8))
    ...
    func chord_basic_emitsBracketedNotes() throws {
        let notes: [ABCNote] = [makeNote(makePitch(.c, .natural, 4), makeLength(1, 1)),
                                makeNote(makePitch(.e, .natural, 4), makeLength(1, 1)),
                                makeNote(makePitch(.g, .natural, 4), makeLength(1, 1))]
        let output = try format(minimalTunebook(symbols: [.chord(makeChord(notes, makeLength(1, 1)))]))

        #expect(output.contains("[CEG]\n"))
    }

    @Test
    func chord_withLengthSuffix_emitsChordSuffix() throws {
        let notes: [ABCNote] = [makeNote(makePitch(.c, .natural, 4), makeLength(1, 1)),
                                makeNote(makePitch(.e, .natural, 4), makeLength(1, 1))]
        let output = try format(minimalTunebook(symbols: [.chord(makeChord(notes, makeLength(2, 1)))]))
    ...
    func chord_withTie_emitsDash() throws {
        let notes: [ABCNote] = [makeNote(makePitch(.c, .natural, 4), makeLength(1, 1)),
                                makeNote(makePitch(.e, .natural, 4), makeLength(1, 1))]
        let output = try format(minimalTunebook(symbols: [.chord(makeChord(notes, makeLength(1, 1), .regular))]))

        #expect(output.contains("[CE]-\n"))
    }

    @Test
    func deprecatedTempo_resolvedToAbsoluteThroughPipeline() throws {
        // Parser leaves Q:C=120 unresolved; the normalizer resolves it against
        // L:1/8 to the absolute Q:1/8=120 that the formatter emits.
        let input = "%abc-2.1\nX:1\nT:Test\nL:1/8\nQ:C=120\nK:C\nCDEF|\n"
        let (parsed, _) = try ABCParser().parse(Data(input.utf8))
        let (normalized, _) = ABCNormalizer().normalize(parsed)
        let (validated, _) = try ABCValidator().validate(normalized)
        let output = try String(data: ABCFormatter().format(validated), encoding: .utf8).require()

        #expect(output.contains("Q:1/8=120\n"))
    }

    @Test
    func field_area_emitsAField() throws {
        let tunebook = makeTunebook([.field(.area("Ireland"))],
                                    [makeTune(header: [.field(.referenceNumber(makeReferenceNumber(1))),
                                                       .field(.tuneTitle("Test")),
                                                       .field(.key(makeKeySignature(.c, .major)))])])

        #expect(try format(tunebook).contains("A:Ireland\n"))
    }

    @Test
    func field_book_emitsBField() throws {
        let tunebook = makeTunebook([.field(.book("My Book"))],
                                    [makeTune(header: [.field(.referenceNumber(makeReferenceNumber(1))),
                                                       .field(.tuneTitle("Test")),
                                                       .field(.key(makeKeySignature(.c, .major)))])])

        #expect(try format(tunebook).contains("B:My Book\n"))
    }

    @Test
    func field_composer_emitsCField() throws {
        let tunebook = makeTunebook([.field(.composer("Bach"))],
                                    [makeTune(header: [.field(.referenceNumber(makeReferenceNumber(1))),
                                                       .field(.tuneTitle("Test")),
                                                       .field(.key(makeKeySignature(.c, .major)))])])

        #expect(try format(tunebook).contains("C:Bach\n"))
    }
    ...
    func field_macro_emitsMacroField() throws {
        let tunebook = makeTunebook([.field(.macro(makeMacro("~G2", "{A}G{F}G")))],
                                    [makeTune(header: [.field(.referenceNumber(makeReferenceNumber(1))),
                                                       .field(.tuneTitle("Test")),
                                                       .field(.key(makeKeySignature(.c, .major)))])])

        #expect(try format(tunebook).contains("m:~G2={A}G{F}G\n"))
    }
    ...
    func field_stringField_escapesPercentSign() throws {
        ...
        #expect(try format(tunebook).contains("T:Foo \\% Bar\n"))
    }
    ...
    func field_unitNoteLength_emitsLField() throws {
        let tunebook = makeTunebook([.field(.unitNoteLength(makeLength(1, 8)))],
                                    [makeTune(header: [.field(.referenceNumber(makeReferenceNumber(1))),
                                                       .field(.tuneTitle("Test")),
                                                       .field(.key(makeKeySignature(.c, .major)))])])

        #expect(try format(tunebook).contains("L:1/8\n"))
    }

    @Test
    func field_userSymbol_annotation_emitsUField() throws {
        let tunebook = makeTunebook([.field(.userDefined(makeUserSymbol(.hUpper, makeAnnotation(.above, "fermata"))))],
                                    [makeTune(header: [.field(.referenceNumber(makeReferenceNumber(1))),
                                                       .field(.tuneTitle("Test")),
                                                       .field(.key(makeKeySignature(.c, .major)))])])

        #expect(try format(tunebook).contains("U:H=\"^fermata\"\n"))
    }

    @Test
    func field_userSymbol_deassignment_emitsNilSentinel() throws {
        let tunebook = makeTunebook([.field(.userDefined(makeUserSymbol(.tUpper)))],
                                    [makeTune(header: [.field(.referenceNumber(makeReferenceNumber(1))),
                                                       .field(.tuneTitle("Test")),
                                                       .field(.key(makeKeySignature(.c, .major)))])])

        #expect(try format(tunebook).contains("U:T=!nil!\n"))
    }

    @Test
    func field_userSymbol_emitsUField() throws {
        let tunebook = makeTunebook([.field(.userDefined(makeUserSymbol(.tilde, makeDecoration("roll"))))],
                                    [makeTune(header: [.field(.referenceNumber(makeReferenceNumber(1))),
                                                       .field(.tuneTitle("Test")),
                                                       .field(.key(makeKeySignature(.c, .major)))])])

        #expect(try format(tunebook).contains("U:~=!roll!\n"))
    }

    @Test
    func fileHeader_beginEndDirective_emitsBlockForm() throws {
        let tunebook = makeTunebook([.directive(makeDirective("text",
                                                              "",
                                                              ["Line one", "Line two"]))],
                                    [makeTune(header: [.field(.referenceNumber(makeReferenceNumber(1))),
                                                       .field(.tuneTitle("Test")),
                                                       .field(.key(makeKeySignature(.c, .major)))])])
        let output = try format(tunebook)
    ...
    func fileHeader_directive_emitsPercentPercent() throws {
        let tunebook = makeTunebook([.directive(makeDirective("midi",
                                                              "program 40"))],
                                    [makeTune(header: [.field(.referenceNumber(makeReferenceNumber(1))),
                                                       .field(.tuneTitle("Test")),
                                                       .field(.key(makeKeySignature(.c, .major)))])])
    ...
    func fileHeader_meterField_emitsMField() throws {
        let tunebook = makeTunebook([.field(.meter(makeTimeSignature(4, 4)))],
                                    [makeTune(header: [.field(.referenceNumber(makeReferenceNumber(1))),
                                                       .field(.tuneTitle("Test")),
                                                       .field(.key(makeKeySignature(.c, .major)))])])
        let output = try format(tunebook)
    ...
    func format_unvalidatedTunebook_throwsNotValidated() {
        ...
        #expect(throws: ABCFormatter.Error.notValidated) {
            try ABCFormatter().format(tunebook)
        }
    }

    @Test
    func graceNotes_noSlash_emitsCurlyBraces() throws {
        let notes: [ABCNote] = [makeNote(makePitch(.a, .natural, 4), makeLength(1, 1))]
        let following = makeNote(makePitch(.g, .natural, 4), makeLength(1, 1))
    ...
    func idempotence_simpleTune_formatsIdentically() throws {
        let input = "%abc-2.1\nX:1\nT:Test\nM:4/4\nL:1/8\nK:G\nGABc defe|\n"
        let (tunebook1, _) = try ABCParser().parse(Data(input.utf8))
        let (normalizedTunebook1, _) = ABCNormalizer().normalize(tunebook1)
        let (validatedTunebook1, _) = try ABCValidator().validate(normalizedTunebook1)
        let formatted1 = try ABCFormatter().format(validatedTunebook1)
        let (tunebook2, _) = try ABCParser().parse(formatted1)
        let (normalizedTunebook2, _) = ABCNormalizer().normalize(tunebook2)
        let (validatedTunebook2, _) = try ABCValidator().validate(normalizedTunebook2)
        let formatted2 = try ABCFormatter().format(validatedTunebook2)
    ...
    @Test
    func length_default_emitsEmpty() throws {
        // A written length of 1/1 (a bare note) emits no suffix.
        let note = makeNote(makePitch(.c, .natural, 4),
                            makeLength(1, 1))
        let output = try format(minimalTunebook(symbols: [.note(note)]))
    ...
    @Test
    func length_double_emits2() throws {
        let note = makeNote(makePitch(.c, .natural, 4),
                            makeLength(2, 1))
        let output = try format(minimalTunebook(symbols: [.note(note)]))
    ...
    @Test
    func length_half_emitsSlash() throws {
        let note = makeNote(makePitch(.c, .natural, 4),
                            makeLength(1, 2))
        let output = try format(minimalTunebook(symbols: [.note(note)]))
    ...
    @Test
    func length_quarter_emitsDoubleSlash() throws {
        let note = makeNote(makePitch(.c, .natural, 4),
                            makeLength(1, 4))
        let output = try format(minimalTunebook(symbols: [.note(note)]))
    ...
    @Test
    func length_threeHalves_emits3over2() throws {
        let note = makeNote(makePitch(.c, .natural, 4),
                            makeLength(3, 2))
        let output = try format(minimalTunebook(symbols: [.note(note)]))
    ...
    func meter_common_emitsC() throws {
        let tunebook = makeTunebook([.field(.meter(.common))],
                                    [makeTune(header: [.field(.referenceNumber(makeReferenceNumber(1))),
                                                       .field(.tuneTitle("Test")),
                                                       .field(.key(makeKeySignature(.c, .major)))])])

        #expect(try format(tunebook).contains("M:C\n"))
    }

    @Test
    func meter_complex_emitsParenthesizedForm() throws {
        let tunebook = makeTunebook([.field(.meter(makeTimeSignature([2, 3, 2], 8)))],
                                    [makeTune(header: [.field(.referenceNumber(makeReferenceNumber(1))),
                                                       .field(.tuneTitle("Test")),
                                                       .field(.key(makeKeySignature(.c, .major)))])])

        #expect(try format(tunebook).contains("M:(2+3+2)/8\n"))
    }

    @Test
    func meter_cut_emitsCPipe() throws {
        let tunebook = makeTunebook([.field(.meter(.cut))],
                                    [makeTune(header: [.field(.referenceNumber(makeReferenceNumber(1))),
                                                       .field(.tuneTitle("Test")),
                                                       .field(.key(makeKeySignature(.c, .major)))])])
    ...
    func meter_empty_emitsNone() throws {
        let tunebook = makeTunebook([.field(.meter(.empty))],
                                    [makeTune(header: [.field(.referenceNumber(makeReferenceNumber(1))),
                                                       .field(.tuneTitle("Test")),
                                                       .field(.key(makeKeySignature(.c, .major)))])])

        #expect(try format(tunebook).contains("M:none\n"))
    }

    @Test
    func meter_standard_emitsFraction() throws {
        let tunebook = makeTunebook([.field(.meter(makeTimeSignature(3, 4)))],
                                    [makeTune(header: [.field(.referenceNumber(makeReferenceNumber(1))),
                                                       .field(.tuneTitle("Test")),
                                                       .field(.key(makeKeySignature(.c, .major)))])])

        #expect(try format(tunebook).contains("M:3/4\n"))
    }
    ...
    func part_inBody_emitsLetter() throws {
        ...
        #expect(try format(tunebook).contains("P:A\n"))
    }
    ...
    func parts_simple_emitsLetters() throws {
        ...
        #expect(try format(tunebook).contains("P:AB\n"))
    }
    ...
    func parts_withGroup_emitsParentheses() throws {
        ...
        #expect(try format(tunebook).contains("P:A(BC)3\n"))
    }
    ...
    func rest_regular_emitsZ() throws {
        let output = try format(minimalTunebook(symbols: [.rest(.regular(false, makeLength(1, 1)))]))
    ...
    func rest_regularInvisible_emitsX() throws {
        let output = try format(minimalTunebook(symbols: [.rest(.regular(true, makeLength(1, 1)))]))
    ...
    func roundTrip_beamBreak_beamedDistinctFromUnbeamed() throws {
        ...
        let (tunebookBeamed, _) = try ABCParser().parse(Data(beamed.utf8))
        let (tunebookUnbeamed, _) = try ABCParser().parse(Data(unbeamed.utf8))

        #expect(tunebookBeamed != tunebookUnbeamed)
    }
    ...
    func roundTrip_beamBreak_preservedAfterFormat() throws {
        ...
        let (normalizedTunebook1, _) = ABCNormalizer().normalize(tunebook1)
        let (validatedTunebook1, _) = try ABCValidator().validate(normalizedTunebook1)
        let formatted = try ABCFormatter().format(validatedTunebook1)
        let (tunebook2, _) = try ABCParser().parse(formatted)

        #expect(tunebook1 == tunebook2)
    }
    ...
    @Test
    func roundTrip_explicitLengths_preserveWrittenFormRegardlessOfL() throws {
        // With note length stored as written, the multiplier survives a
        // parse → format round-trip unchanged, independent of L:.
        let input = "%abc-2.1\nX:1\nT:Test\nM:4/4\nL:1/4\nK:G\nA2 B/ c3/2 d|\n"
        let (tunebook, _) = try ABCParser().parse(Data(input.utf8))

        #expect(try format(tunebook).contains("A2 B/ c3/2 d|\n"))
    }
    ...
    func spacer_emitsY() throws {
        let output = try format(minimalTunebook(symbols: [.spacer(makeLength(1, 1))]))

        #expect(output.contains("y\n"))
    }

    @Test
    func spacer_withLength_emitsYWithSuffix() throws {
        let output = try format(minimalTunebook(symbols: [.spacer(makeLength(2, 1))]))
    ...
    func symbolLine_skip_emitsStar() throws {
        ...
        #expect(try format(tunebook).contains("s:* !trill! *\n"))
    }

    @Test
    func tempo_lengthAndRate_emitsFullForm() throws {
        #expect(try format(minimalTunebookWithTempo(makeTempo(1, 4, 120))).contains("Q:1/4=120\n"))
    }

    @Test
    func tempo_lengthTextAndRate_emitsFullForm() throws {
    ...
    func tune_missingKeySignature_outputsWithoutKeyField() throws {
        ...
        // A tune with no K: field is not spec-conformant and so cannot pass
        // validation; format a hand-validated copy directly to exercise the
        // formatter's handling of a missing key field in isolation.
        let (normalized, _) = ABCNormalizer().normalize(tunebook)
        let validated = ABCTunebook(version: normalized.version,
                                    fileHeader: normalized.fileHeader,
                                    tunes: normalized.tunes,
                                    isNormalized: normalized.isNormalized,
                                    isValidated: true)
        let output = try String(bytes: ABCFormatter().format(validated),
    ...
    func variantEnding_abbreviatedForm_normalizesToCanonical() throws {
        let input = "%abc-2.1\nX:1\nT:Test\nL:1/8\nK:C\n|:CDEF:|2 cdef|]\n"
        let (tunebook, _) = try ABCParser().parse(Data(input.utf8))
        let formatted = try format(tunebook)
    ...
    func voice_simple_emitsIDOnly() throws {
        ...
        #expect(try format(tunebook).contains("V:V1\n"))
    }
