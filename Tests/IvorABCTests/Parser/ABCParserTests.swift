...
extension ABCParserTests {
    ...
    func parse_alignedLyrics_textEscapes_roundTrip() throws {
        ...
        if case let .field(.wordsAligned(al)) = lyricsEntry {
            #expect(al == makeAlignedWords([.syllable("fó"),
                                            .syllable("ü"),
                                            .continuation,
                                            .syllable("zy"),
                                            .syllable("foo%bar")]))
    ...
    func parse_lineContinuation_fieldLineBreaksAccumulation() throws {
        ...
        let lyricsEntries = tune.body.filter { if case .field(.wordsAligned) = $0 { true } else { false } }
    ...
    func parse_lineContinuation_joinsContinuedLines() throws {
        ...
        let symbolEntries = tune.body.filter {
            if case .symbols = $0 {
                return true
            }
    ...
    func parse_percentEscape_roundTrips() throws {
        ...
        if case let .field(.tuneTitle(text)) = titleEntry {
            #expect(text == "Foo % Bar")
        } else {
