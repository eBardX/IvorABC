...
extension ABCAlignedLyricsTests {
    @Test
    func equality() {
        let al1 = makeAlignedLyrics([.text("grace"), .hold])
        let al2 = makeAlignedLyrics([.text("grace"), .hold])
    ...
    func inequality_differentCount() {
        let al1 = makeAlignedLyrics([.text("grace")])
        let al2 = makeAlignedLyrics([.text("grace"), .hold])
    ...
    @Test
    func segments_mixed() {
        let al = makeAlignedLyrics([.text("A"), .hyphen, .text("ma"), .hold, .skip, .barAlign])
