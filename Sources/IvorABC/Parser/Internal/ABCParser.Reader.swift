...
extension ABCParser.Reader {
    ...
    internal mutating func readTunebook() throws(ABCParser.Error) -> (ABCTunebook, [ABCParser.Diagnostic]) {
    ...
    private func _readDirective(_ tidyInput: Substring) throws(ABCParser.Error) -> ABCDirective {
    ...
    private func _readDirectiveLine(_ input: Substring) throws(ABCParser.Error) -> ABCParser.Line? {
    ...
    private mutating func _readFieldLine(_ input: Substring,
                                         _ policy: ABCParser.Policy) throws(ABCParser.Error) -> ABCParser.Line? {
    ...
    private func _readInstructionLine(_ input: Substring) throws(ABCParser.Error) -> ABCParser.Line {
        let tidyInput = trimSuffix(uncomment(input.dropFirst(2)))
        let result = tidyInput.splitBeforeFirst { $0.isABCWhitespace }
    ...
    private mutating func _readLine(_ input: Substring,
                                    _ policy: ABCParser.Policy) throws(ABCParser.Error) -> ABCParser.Line? {
        if let line = _readEmptyLine(input) {
            return line
        }

        if let line = try _readDirectiveLine(input) {
            return line
        }

        if let line = try _readFieldLine(input, policy) {
            return line
        }

        return try _readSymbolsLine(input)
    }

    private func _readSymbolsLine(_ input: Substring) throws(ABCParser.Error) -> ABCParser.Line? {
        let tokens = try tokenizer.tokenize(String(input))
        var matcher = ABCSymbolMatcher(tokens: tokens)
        let symbols = try matcher.matchSymbols()
    ...
    private mutating func _readTunebook() throws(ABCParser.Error) -> ABCTunebook {
