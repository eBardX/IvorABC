...
///
/// See §4.6 (“Clefs and transposition”) of the ABC 2.1 standard.
...
extension ABCClef {
    ...
    /// Returns the staff line on which the named clef is placed by default.
    ///
    /// These are the defaults defined by the ABC 2.1 standard: treble is `2`,
    /// alto is `3`, bass and tenor are `4`, and everything else — including an
    /// unspecified name — is `1`. ``init(name:line:ottava:middle:transpose:octave:stafflines:)``
    /// applies this value whenever `line` is `nil`.
    ///
    /// Use this to distinguish a ``line`` that was written in the source from
    /// one that was merely defaulted, since ``line`` itself is always resolved
    /// to a concrete value.
    ///
    /// - Parameter name: The clef name, or `nil` if unspecified.
    ///
    /// - Returns: The default staff line for `name`.
    public static func defaultLine(for name: Name?) -> Int {
